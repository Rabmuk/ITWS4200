...
app.get('/', function(req, res){
  ...
  html += '<p>View the mongo data at: <form action="mongo"> <input type="submit" value="Go to Mongo"> </form> </p>';
  html += '<p>Lab 1 rework at: <form action="scrolling"> <input type="submit" value="Scrolling"> </form> </p>';
...
twit.stream('statuses/filter', {'locations':sw +','+ne},
  function(stream) {
    ...
    MongoClient.connect("mongodb://localhost:27017/lab7", function(err, db) { 
      ...
      db.collection('tweets',function(err, table){
       table.remove({},function(err, removed){
      });
});
    });
    stream.on('data', function (response) {
      var doc = {key:count, data:response};
...
{
  var html = '<h2>Data from mongo database:</h2>'; 
...
app.get('/update.js', function(req, res){
  var file = __dirname + '/update.js';
  res.download(file);
});

app.get('/style.css', function(req, res){
  var file = __dirname + '/style.css';
  res.download(file);
});
